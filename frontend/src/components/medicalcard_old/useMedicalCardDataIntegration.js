...
export const useMedicalCardDataIntegration = (patient) => {
  ...
  const getIntegratedMedicalData = useCallback((patientId, modules, storedData = {}) => {
    ...
    if (modules) {
      ...
      if (modules.photometry && modules.photometry.data) {
        ...
        // Интеграция данных будет в отдельном файле
        // для краткости опускаем полную логику интеграции
        integratedData.photoAnalysis.detailedPhotometry = photoData;
      }
